...
trait OrgaCommentRepo {
  ...
  def getComments(proposal: Proposal.Id)(implicit ctx: UserCtx): IO[List[Comment.Full]]

  def getOrgaComments(proposal: Proposal.Id)(implicit ctx: OrgaCtx): IO[List[Comment.Full]]
}
...
trait PublicCommentRepo {
  ...
  def getComments(event: Event.Id): IO[List[Comment.Full]]
}
