...
object HttpUtils {
  ...
  def getReferer(req: GsRequest[AnyContent]): Option[String] = getReferer(req.headers)

  def getReferer(headers: Headers): Option[String] = headers.get("Referer")

  // ex: /u/groups/ht-paris/events/2019-02/edit
  def getRequestUri(req: GsRequest[AnyContent]): Option[String] = getRequestUri(req.headers)
  ...
  def cancelUrl(req: GsRequest[AnyContent], call: => Call): String = {
