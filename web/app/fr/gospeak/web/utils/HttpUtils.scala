...
import play.api.mvc.{AnyContent, Call, Headers, Request}

import scala.util.Try

object HttpUtils {
  // ex: http://localhost:9000/u/groups/ht-paris/events/2019-02/edit
  def getReferer(req: Request[AnyContent]): Option[String] = getReferer(req.headers)

  def getReferer(headers: Headers): Option[String] = headers.get("Referer")

  // ex: /u/groups/ht-paris/events/2019-02/edit
  def getRequestUri(req: Request[AnyContent]): Option[String] = getRequestUri(req.headers)
