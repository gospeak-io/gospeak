...
class MessageBuilder {
  ...
  def buildEventInfo(group: Group, event: Event, cfpOpt: Option[Cfp], venueOpt: Option[Venue.Full], talks: Seq[Proposal], speakers: Seq[User])(implicit req: SecuredReq[AnyContent]): TemplateData.EventInfo = {
  ...
  def buildEventInfo(i: EventFull)(implicit req: SecuredReq[AnyContent]): TemplateData.EventInfo =
    buildEventInfo(i.group, i.event, i.cfpOpt, i.venueOpt, i.talks, i.speakers)

  private def linked(group: Group)(implicit req: SecuredReq[AnyContent]): Linked[Group] = {
  ...
  private def linked(group: Group, event: Event)(implicit req: SecuredReq[AnyContent]): Linked[Event] = {
  ...
  private def linked(group: Group, cfp: Cfp)(implicit req: SecuredReq[AnyContent]): Linked[Cfp] = {
  ...
  private def linked(group: Group, event: Event, cfp: Cfp, proposal: Proposal)(implicit req: SecuredReq[AnyContent]): Linked[Proposal] = {
    val link = fr.gospeak.web.pages.orga.cfps.proposals.routes.ProposalCtrl.detail(group.slug, cfp.slug, proposal.id).absoluteURL()
    val publicLink = if (event.isPublic) {
  ...
  private def linked(group: Group, user: User)(implicit req: SecuredReq[AnyContent]): Linked[User] = {
