...
object PageMeta {
  def default()(implicit req: BasicReq[AnyContent]): PageMeta = PageMeta(
    ...
    image = "https://gospeak.io/logo.png",
  ...
  def group(g: Group)(implicit req: BasicReq[AnyContent]): PageMeta = PageMeta(
    ...
    image = g.logo.map(_.value).getOrElse("https://gospeak.io/logo.png"),
  ...
  def cfp(c: ExternalCfp)(implicit req: BasicReq[AnyContent]): PageMeta = PageMeta(
    title = c.name.value.take(70),
    description = c.description.value.take(200),
    image = c.logo.map(_.value).getOrElse("https://gospeak.io/logo.png"),
