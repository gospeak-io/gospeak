...
class CfpCtrlSpec extends CtrlSpec with BeforeAndAfterEach {
  ...
  private val ctrl = new CfpCtrl(cc, silhouette, conf.application.env, db.group, db.cfp, db.talk, db.proposal, db.userRequest, db.externalCfp, authSrv, emailSrv, orgaMessageBus, timeShape)
