  ...
  ignore("YoutubeClient") {
    ...
    describe("getChannelId") {
      it("should get channel id with user name") {
        ...
        channel.value shouldBe "UCVelKVoLQIhwx9C2LWf-CDA"
      }
      it("should get channel id with channel id") {
        ...
        val channel = client.getChannelId(url).unsafeRunSync().get
        channel.value shouldBe "UCKFAwlgWiAB4vUpgnS63qog"
      }
    ...
    describe("getPlaylist") {
      it("should get playlist with id") {
        ...
        playlist.id.value shouldBe "PLs13l-4BLe9cKAJEZJoh5u1UQMACty7Xi"
        playlist.title shouldBe "Octobre 2019"
        playlist.channelTitle shouldBe "Human Talks Paris"
        playlist.items shouldBe 4
      }
    }
    describe("listChannelVideos") {
      it("should list channel videos") {
        ...
        videos.items.map(_.id.value) should contain allElementsOf List("JyNq_-OJ3dA", "5JjcxOfCSl0")
      }
    ...
    describe("listPlaylistVideos") {
      it("should list playlist videos") {
        ...
        videos.items.map(_.id.value) shouldBe List("7wf3NDUq1jw", "9J9ouo-VNao", "r3xdCYP9mVs", "AoVFq4rqv5g")
      }
      it("should fail on bad id") {
