...
object ExternalCfpRepoSql {
  ...
  private val commonTable = Table(
    name = "((SELECT id,         slug, name, begin, close, description, tags FROM cfps) " +
      "UNION (SELECT id, null as slug, name, begin, close, description, tags FROM external_cfps))",
    prefix = "c",
    joins = Seq(),
    fields = Seq("id", "slug", "name", "begin", "close", "description", "tags").map(Field(_, "c")),
  ...
  private[sql] def insert(e: ExternalCfp): Insert[ExternalCfp] = {
    val values = fr0"${e.id}, ${e.name}, ${e.logo}, ${e.description}, ${e.begin}, ${e.close}, ${e.url}, ${e.event.start}, ${e.event.finish}, ${e.event.url}, " ++ insertAddress(e.event.address) ++ fr0", ${e.event.tickets}, ${e.event.videos}, ${e.event.twitterAccount}, ${e.event.twitterHashtag}, ${e.tags}, ${e.info.created}, ${e.info.createdBy}, ${e.info.updated}, ${e.info.updatedBy}"
  ...
  private[sql] def update(cfp: ExternalCfp.Id)(e: ExternalCfp.Data, by: User.Id, now: Instant): Update = {
    val fields = fr0"name=${e.name}, logo=${e.logo}, description=${e.description}, begin=${e.begin}, close=${e.close}, url=${e.url}, event_start=${e.event.start}, event_finish=${e.event.finish}, event_url=${e.event.url}, " ++ updateAddress(e.event.address) ++ fr0", tickets_url=${e.event.tickets}, videos_url=${e.event.videos}, twitter_account=${e.event.twitterAccount}, twitter_hashtag=${e.event.twitterHashtag}, tags=${e.tags}, updated_at=$now, updated_by=$by"
  ...
  private def insertAddress(a: Option[GMapPlace]) =
    fr0"$a, ${a.map(_.geo.lat)}, ${a.map(_.geo.lng)}, ${a.flatMap(_.locality)}, ${a.map(_.country)}"

  private def updateAddress(a: Option[GMapPlace]) =
    fr0"address=$a, address_lat=${a.map(_.geo.lat)}, address_lng=${a.map(_.geo.lng)}, address_locality=${a.flatMap(_.locality)}, address_country=${a.map(_.country)}"
}
