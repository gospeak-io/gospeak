...
class GospeakDbSql extends GospeakDb {
  ...
  private val events = mutable.ArrayBuffer(
    Event(event1, Event.Slug("2019-03"), group1, Event.Name("HumanTalks Paris Mars 2019"), Some("desc"), Some("Zeenea"), Seq(proposal1)),
    Event(event2, Event.Slug("2019-04"), group1, Event.Name("HumanTalks Paris Avril 2019"), None, None, Seq()),
    Event(event3, Event.Slug("2019-03"), group2, Event.Name("Paris.Js Avril"), None, None, Seq()),
    Event(event4, Event.Slug("2019-03"), group3, Event.Name("Nouveaux modèles de gouvenance"), None, None, Seq()))
  ...
  override def createEvent(group: Group.Id, slug: Event.Slug, name: Event.Name): Future[Event] = {
    val event = Event(Event.Id.generate(), slug, group, name, None, None, Seq())
    events += event
