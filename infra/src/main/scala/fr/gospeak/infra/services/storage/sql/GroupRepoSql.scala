...
object GroupRepoSql {
  ...
  private[sql] def insert(e: Group): Insert[Group] = {
    val values = fr0"${e.id}, ${e.slug}, ${e.name}, ${e.logo}, ${e.banner}, ${e.contact}, ${e.website}, ${e.description}, ${e.location}, ${e.location.map(_.geo.lat)}, ${e.location.map(_.geo.lng)}, ${e.location.map(_.country)}, ${e.owners}, " ++
      ...
      fr0"${e.tags}, ${e.status}, ${e.info.created}, ${e.info.createdBy}, ${e.info.updated}, ${e.info.updatedBy}"
    table.insert[Group](e, _ => values)
  }

  private[sql] def update(group: Group.Slug)(d: Group.Data, by: User.Id, now: Instant): Update = {
    val fields = fr0"slug=${d.slug}, name=${d.name}, logo=${d.logo}, banner=${d.banner}, contact=${d.contact}, website=${d.website}, description=${d.description}, location=${d.location}, location_lat=${d.location.map(_.geo.lat)}, location_lng=${d.location.map(_.geo.lng)}, location_country=${d.location.map(_.country)}, " ++
      ...
      fr0"tags=${d.tags}, updated=$now, updated_by=$by"
