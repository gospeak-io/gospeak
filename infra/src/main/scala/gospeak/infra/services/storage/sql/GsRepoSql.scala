...
class GsRepoSql(dbConf: DbConf, gsConf: GsConf) extends GsRepo {
  ...
  def insertMockData(): IO[Done] = {
    val _ = eventIdMeta // for intellij not remove DoobieMappings import
    var n = Instant.now()
    ...
    def group(slug: String, name: String, tags: List[String], by: User, location: Option[GMapPlace] = None, owners: List[User] = List(), logo: Option[String] = None, social: SocialAccounts = SocialAccounts.fromUrls(), email: Option[String] = None): Group =
      Group(Group.Id.generate(), Group.Slug.from(slug).get, Group.Name(name), logo.map(u => Logo(Url.from(u).get)), None, email.map(EmailAddress.from(_).get), None, description = Markdown("Cras sit amet nibh libero, in gravida nulla. Nulla vel metus scelerisque ante sollicitudin."), location, owners = NonEmptyList.of(by.id) ++ owners.map(_.id), social, tags = tags.map(Tag(_)), Group.Status.Active, info = Info(by.id, now))

    def cfp(group: Group, slug: String, name: String, start: Option[String], end: Option[String], description: String, tags: List[String], by: User): Cfp =
    ...
    def talk(by: User, slug: String, title: String, status: Talk.Status = Talk.Status.Public, speakers: List[User] = List(), duration: Int = 10, slides: Option[String] = None, video: Option[String] = None, description: String = "Cras sit amet nibh libero, in gravida nulla. Nulla vel metus scelerisque ante sollicitudin.", tags: List[String] = List()): Talk =
      Talk(Talk.Id.generate(), Talk.Slug.from(slug).get, status, Talk.Title(title), Duration(duration, MINUTES), Markdown(description), Markdown(""), NonEmptyList.of(by.id) ++ speakers.map(_.id), slides.map(Url.Slides.from(_).get), video.map(Url.Video.from(_).get), tags.map(Tag(_)), Info(by.id, now))

    def proposal(talk: Talk, cfp: Cfp, status: Proposal.Status = Proposal.Status.Pending, orgaTags: List[String] = List()): Proposal =
    ...
    def event(group: Group, cfp: Option[Cfp], slug: String, name: String, date: String, by: User, maxAttendee: Option[Int], allowRsvp: Boolean = false, venue: Option[Venue] = None, description: LiquidMarkdown[Message.EventInfo] = LiquidMarkdown[Message.EventInfo](""), tags: List[String] = List(), published: Boolean = true): Event =
      Event(Event.Id.generate(), group.id, cfp.map(_.id), Event.Slug.from(slug).get, Event.Name(name), Event.Kind.Meetup, LocalDateTime.parse(s"${date}T19:00:00"), maxAttendee, allowRsvp, description, Event.Notes("", now, by.id), venue.map(_.id), List(), tags.map(Tag(_)), if (published) Some(Instant.parse(date + "T06:06:24.074Z")) else None, Event.ExtRefs(), Info(by.id, now))
    ...
    def eventExt(name: String, start: Option[String], url: Option[String] = None, logo: Option[String] = None, description: String = "", finish: Option[String] = None, location: Option[GMapPlace] = None, ticketsUrl: Option[String] = None, videosUrl: Option[String] = None, twitterAccount: Option[String] = None, twitterHashtag: Option[String] = None, tags: List[String] = List(), by: User): ExternalEvent =
    ...
    def video(url: String, channel: (String, String), playlist: Option[(String, String)], title: String, description: String, tags: List[String], published: String, duration: Int, lang: String, views: Long, likes: Long, dislikes: Long, comments: Long): Video =
    ...
    val userDemo = user(
      ...
      mentoring = Some(Markdown("I can review you **CFP abstract** and **slides**")),
      company = Some("Zeenea"),
      location = Some("Paris"),
    ...
    val users = List(userDemo, userSpeaker, userOrga, userEmpty, lkn)
    ...
    val whyFP = talk(userDemo, "why-fp", "Why FP", status = Talk.Status.Public, tags = List("FP"),
    ...
    val scalaBestPractices = talk(userDemo, "scala-best-practices", "Scala Best Practices", speakers = List(userSpeaker),
      slides = Some("https://docs.google.com/presentation/d/1wWRKbxz81AzhBJJqc505yUkileRPn5b-bNH1Th852f4"),
      video = Some("https://www.youtube.com/watch?v=Tm-qyMukBq4"),
      description =
        """I have seen a lot of people struggleing with Scala because they were lost in all the feature and did not know which one to use and *not to use*.
          ...
          |- never use null
          |- go functional
        """.stripMargin.trim, tags = List("tech", "scala", "beginner"))
    val nodeNews = talk(userDemo, "nodejs-news", "NodeJs news", status = Talk.Status.Private)
    val scalaReact = talk(userSpeaker, "scalajs-react", "ScalaJS + React = <3", speakers = List(userDemo), duration = 50, tags = List("Scala"))
    ...
    val talks = List(whyFP, scalaBestPractices, nodeNews, scalaReact, akkaBlackJack, startWithSpark, bigTalk)

    val humanTalks = group("ht-paris", "HumanTalks Paris", List("tech"), userDemo, location = Some(parisPlace), owners = List(userOrga), email = Some("paris@humantalks.com"), logo = Some("https://res.cloudinary.com/gospeak/image/upload/ar_1,c_crop/v1576793051/groups/humantalks-paris_7bf00e98-2298-47b1-a001-35e5e307249f/logo.png"), social = social)
    val parisJs = group("paris-js", "Paris.Js", List("JavaScript"), userOrga)
    val dataGov = group("data-gov", "Data governance", List(), userDemo)
    val bigGroup = group("big-group", "Big Group", List("BigData"), userOrga)
    val groups = List(humanTalks, parisJs, dataGov, bigGroup)

    val humanTalksSettings = groupDefaultSettings.copy(
      actions = Map(
        Group.Settings.Action.Trigger.OnEventCreated -> List(
    ...
    val cfp1 = cfp(humanTalks, "ht-paris", "HumanTalks Paris", None, None, "Les HumanTalks Paris c'est 4 talks de 10 min...", List("tag1", "tag2"), userDemo)
    val cfp2 = cfp(humanTalks, "ht-paris-day-1", "HumanTalks Paris Day - Edition 1", None, Some("2018-07-01"), "Les HumanTalks Paris c'est 4 talks de 10 min...", List(), userDemo)
    val cfp3 = cfp(humanTalks, "ht-paris-day-2", "HumanTalks Paris Day - Edition 2", Some("2019-03-01"), Some("2019-12-31"), "Les HumanTalks Paris c'est 4 talks de 10 min...", List(), userDemo)
    val cfp4 = cfp(parisJs, "paris-js", "Paris.Js", None, Some("2019-05-21"), "Submit your talk to exchange with the Paris JS community", List(), userOrga)
    val cfps = List(cfp1, cfp2, cfp3, cfp4)

    val proposal1 = proposal(whyFP, cfp1)
    val proposal2 = proposal(scalaBestPractices, cfp1, orgaTags = List("accepted", "1st round"))
    ...
    val proposals = List(proposal1, proposal2, proposal3, proposal4, proposal5)
    ...
    val partners = List(zeeneaHT, criteo, nexeo, google, zeeneaDG, voodoo)
    ...
    val contacts = List(zeeneaHTNina, zeeneaHTJean, criteoClaude, zeeneaDGNina)
    ...
    val event6 = event(dataGov, None, "2019-03", "Nouveaux modeles de gouvenance", "2019-03-15", userDemo, Some(100), venue = Some(venue2), tags = List("Data Gouv"))
    val event9 = event(dataGov, None, "2019-04", "Nouveaux modeles de gouvenance", "2019-04-15", userDemo, Some(100), venue = Some(venue3), tags = List("Data Gouv"))
    val events = List(event1, event2, event3, event4, event5, event6, event7, event8, event9)

    val eventTalks = List(
      (cfp1, event4, List(proposal1), humanTalks.owners.head),
      (cfp1, event8, List(proposal2, proposal4), humanTalks.owners.head),
      (cfp4, event5, List(proposal3), parisJs.owners.head))
    ...
    val packs = List(base, premium, old)
    ...
    val cfpExts = List(cfpDevoxx2020)

    val whyFPDevoxx2020 = proposalExt(whyFP, devoxx2020)
    val proposalExts = List(whyFPDevoxx2020)
    ...
    val microservices = video(
      ...
      title = "Découvrir par l’exemple : Microservices et Event Sourcing avec Kafka et Kubernetes - Tugdual Grall", description = "", tags = List(),
    ...
    val micode = video(
      url = "https://www.youtube.com/watch?v=crnCudRa3wc", channel = "Micode" -> "UCYnvxJ-PKiGXo_tYXpWAC-w", playlist = None, title = "HACKER SHADOW EN 24H !", description =
        """Ma marque : https://foreach.shop
          ...
          |""".stripMargin.trim,
      tags = List("micode", "shadow pc", "shadow", "hacker", "hacker shadow", "24h", "en 24h", "mot de passe", "hack", "cloud gaming", "cloud computing"),
      published = "2019-12-03", duration = 639, lang = "fr", views = 375859, likes = 21785, dislikes = 451, comments = 900)
    val videos = List(microservices, micode)

    val extEventVideos = List(codeursenseine2018.id -> List(microservices.id))

    val generated = (1 to 25).map { i =>
      val groupId = Group.Id.generate()
      val cfpId = Cfp.Id.generate()
      val g = Group(groupId, Group.Slug.from(s"z-group-$i").get, Group.Name(s"Z Group $i"), None, None, None, None, Markdown("Cras sit amet nibh libero, in gravida nulla. Nulla vel metus scelerisque ante sollicitudin."), None, NonEmptyList.of(userOrga.id), SocialAccounts.fromUrls(), List(), Group.Status.Active, Info(userOrga.id, now))
      val c = Cfp(cfpId, groupId, Cfp.Slug.from(s"z-cfp-$i").get, Cfp.Name(s"Z CFP $i"), None, None, Markdown("Only your best talks !"), List(), Info(userOrga.id, now))
      val e = Event(Event.Id.generate(), bigGroup.id, None, Event.Slug.from(s"z-event-$i").get, Event.Name(s"Z Event $i"), Event.Kind.Meetup, LocalDateTime.parse("2019-03-12T19:00:00"), Some(100), allowRsvp = false, LiquidMarkdown(""), Event.Notes("", now, userOrga.id), None, List(), List(), Some(now), Event.ExtRefs(), Info(userOrga.id, now))
      val t = Talk(Talk.Id.generate(), Talk.Slug.from(s"z-talk-$i").get, Talk.Status.Public, Talk.Title(s"Z Talk $i"), Duration(10, MINUTES), Markdown("Cras sit amet nibh libero, in gravida nulla. Nulla vel metus scelerisque ante sollicitudin."), Markdown(""), NonEmptyList.of(userSpeaker.id), None, None, List(), Info(userSpeaker.id, now))
      val p = Proposal(Proposal.Id.generate(), bigTalk.id, cfpId, None, Proposal.Status.Pending, Talk.Title(s"Z Proposal $i"), Duration(10, MINUTES), Markdown("temporary description"), Markdown(""), NonEmptyList.of(userSpeaker.id), None, None, List(), List(), Info(userSpeaker.id, now))
  ...
  private def addTalk(cfp: Cfp, event: Event, proposals: List[Proposal], by: User.Id, now: Instant): IO[Done] = for {
