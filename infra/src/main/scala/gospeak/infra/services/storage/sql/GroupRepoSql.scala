...
object GroupRepoSql {
  ...
  private[sql] def insert(e: Group): Query.Insert[GROUPS] = {
    ...
    val q2 = GROUPS.insert.values(values)
  ...
  private[sql] def selectOne(group: Group.Id): Query.Select.Optional[Group] = {
    ...
    val q2 = GROUPS_SELECT.select.where(_.id is group).option[Group]
    GenericRepo.assertEqual(q1.fr, q2.fr)
    q2
  }
  ...
  private[sql] def selectOne(group: Group.Slug): Query.Select.Optional[Group] = {
    ...
    val q2 = GROUPS_SELECT.select.where(_.slug is group).option[Group]
    GenericRepo.assertEqual(q1.fr, q2.fr)
    q2
  }
  ...
  private[sql] def selectOneFull(group: Group.Slug): Query.Select.Optional[Group.Full] = {
    ...
    val q2 = GROUPS_FULL.select.where(GROUPS.SLUG is group).option[Group.Full]
    GenericRepo.assertEqual(q1.fr, q2.fr)
    q2
  }
  ...
  private[sql] def insertMember(m: Group.Member): Query.Insert[GROUP_MEMBERS] = {
    val values = fr0"${m.group}, ${m.user.id}, ${m.role}, ${m.presentation}, ${m.joinedAt}, ${m.leavedAt}"
    val q1 = memberTable.insert[Group.Member](m, _ => values)
    // val q2 = GROUP_MEMBERS.insert.values(m.group, m.user.id, m.role, m.presentation, m.joinedAt, m.leavedAt)
    val q2 = GROUP_MEMBERS.insert.values(values)
    GenericRepo.assertEqual(q1.fr, q2.fr)
    q2
  }
  ...
  private[sql] def disableMember(m: Group.Member, now: Instant): Query.Update[GROUP_MEMBERS] = {
    ...
    val q2 = GROUP_MEMBERS.update.set(_.LEAVED_AT, now).where(gm => gm.GROUP_ID.is(m.group) and gm.USER_ID.is(m.user.id))
    GenericRepo.assertEqual(q1.fr, q2.fr)
    q2
  }
