...
class GROUP_SETTINGS private(getAlias: Option[String] = Some("gs")) extends Table.SqlTable("PUBLIC", "group_settings", getAlias) {
  ...
  val MEETUP_ACCESS_TOKEN: SqlFieldRaw[Crypted, GROUP_SETTINGS] = SqlField(this, "meetup_access_token", "VARCHAR(200)", JdbcType.VarChar, nullable = true, 2)
  val MEETUP_REFRESH_TOKEN: SqlFieldRaw[Crypted, GROUP_SETTINGS] = SqlField(this, "meetup_refresh_token", "VARCHAR(200)", JdbcType.VarChar, nullable = true, 3)
  val MEETUP_GROUP_SLUG: SqlFieldRaw[MeetupGroup.Slug, GROUP_SETTINGS] = SqlField(this, "meetup_group_slug", "VARCHAR(120)", JdbcType.VarChar, nullable = true, 4)
  val MEETUP_LOGGED_USER_ID: SqlFieldRaw[MeetupUser.Id, GROUP_SETTINGS] = SqlField(this, "meetup_logged_user_id", "BIGINT", JdbcType.BigInt, nullable = true, 5)
  val MEETUP_LOGGED_USER_NAME: SqlFieldRaw[String, GROUP_SETTINGS] = SqlField(this, "meetup_logged_user_name", "VARCHAR(120)", JdbcType.VarChar, nullable = true, 6)
  val SLACK_TOKEN: SqlFieldRaw[SlackToken, GROUP_SETTINGS] = SqlField(this, "slack_token", "VARCHAR(200)", JdbcType.VarChar, nullable = true, 7)
  val SLACK_BOT_NAME: SqlFieldRaw[String, GROUP_SETTINGS] = SqlField(this, "slack_bot_name", "VARCHAR(120)", JdbcType.VarChar, nullable = true, 8)
  val SLACK_BOT_AVATAR: SqlFieldRaw[Avatar, GROUP_SETTINGS] = SqlField(this, "slack_bot_avatar", "VARCHAR(1024)", JdbcType.VarChar, nullable = true, 9)
  val EVENT_DESCRIPTION: SqlFieldRaw[LiquidMarkdown[Message.EventInfo], GROUP_SETTINGS] = SqlField(this, "event_description", "VARCHAR NOT NULL", JdbcType.VarChar, nullable = false, 10)
  val EVENT_TEMPLATES: SqlFieldRaw[Map[String, Liquid[Message.EventInfo]], GROUP_SETTINGS] = SqlField(this, "event_templates", "VARCHAR NOT NULL", JdbcType.VarChar, nullable = false, 11)
  val PROPOSAL_TWEET: SqlFieldRaw[Liquid[Message.ProposalInfo], GROUP_SETTINGS] = SqlField(this, "proposal_tweet", "VARCHAR DEFAULT 'Presentation of \"{{proposal.title}}\" by{{#proposal.speakers}}{{^-first}} and{{/-first}} {{#links.twitter}}{{handle}}{{/links.twitter}}{{^links.twitter}}{{name}}{{/links.twitter}}{{/proposal.speakers}}' NOT NULL", JdbcType.VarChar, nullable = false, 15)
  val ACTIONS: SqlFieldRaw[Map[Group.Settings.Action.Trigger, List[Group.Settings.Action]], GROUP_SETTINGS] = SqlField(this, "actions", "VARCHAR NOT NULL", JdbcType.VarChar, nullable = false, 12)
  val UPDATED_AT: SqlFieldRaw[Instant, GROUP_SETTINGS] = SqlField(this, "updated_at", "TIMESTAMP NOT NULL", JdbcType.Timestamp, nullable = false, 13)
