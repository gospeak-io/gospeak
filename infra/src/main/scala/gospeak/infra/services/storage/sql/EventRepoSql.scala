...
object EventRepoSql {
  ...
  private[sql] def insert(e: Event): Query.Insert[EVENTS] = {
    ...
    val q2 = EVENTS.insert.values(values)
  ...
  private[sql] def updateCfp(group: Group.Id, event: Event.Slug)(cfp: Cfp.Id, by: User.Id, now: Instant): Query.Update[EVENTS] = {
    ...
    val q2 = EVENTS.update.set(_.CFP_ID, cfp).set(_.UPDATED_AT, now).set(_.UPDATED_BY, by).where(where2(group, event))
    GenericRepo.assertEqual(q1.fr, q2.fr)
    q2
  }
  ...
  private[sql] def updatePublished(group: Group.Id, event: Event.Slug)(by: User.Id, now: Instant): Query.Update[EVENTS] = {
    ...
    val q2 = EVENTS.update.set(_.PUBLISHED, now).set(_.UPDATED_AT, now).set(_.UPDATED_BY, by).where(where2(group, event))
    GenericRepo.assertEqual(q1.fr, q2.fr)
    q2
  }
