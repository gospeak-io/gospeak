...
class YoutubeSrvImplSpec extends BaseSpec {
  ...
  describe("videos") {
    it("should get all videos by channel id") {
      ...
      val now = Instant.now()
      val res = youtubeSrvImpl.videos(ChannelRef("UCVelKVoLQIhwx9C2LWf-CDA", ""))(now).unsafeRunSync()
    ...
    it("should get all videos by playlist id") {
      ...
      val now = Instant.now()
      val res = youtubeSrvImpl.videos(PlaylistRef("PLv7xGPH0RMUTbzjcYSIMxGXA8RrQWdYGh", ""))(now).unsafeRunSync()

      res.right.get shouldBe (Seq(Video(Url.Video.from("https://www.youtube.com/watch?v=NkH9WNE0OJc").right.get,
