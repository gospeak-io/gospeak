  ...
  describe("UserRepoSql") {
    ...
    describe("Queries") {
      ...
      it("should build update") {
        ...
        check(q, s"UPDATE $table SET slug=?, status=?, first_name=?, last_name=?, email=?, avatar=?, title=?, bio=?, mentoring=?, company=?, location=?, phone=?, website=?, " +
      ...
      it("should build selectAllPublicSlugs") {
        val q = UserRepoSql.selectAllPublicSlugs()
        check(q, s"SELECT u.id, u.slug FROM $table WHERE u.status=? $orderBy")
      }
