...
class ExternalCfpRepoSqlSpec extends RepoSpec {
  ...
  describe("ExternalCfpRepoSql") {
    ...
    it("should select a page") {
      ...
      val cfp1 = externalCfpRepo.create(event1.id, externalCfpData1.copy(close = Some(nowLDT.minusDays(10))))(ctx).unsafeRunSync()
      val cfp2 = externalCfpRepo.create(event2.id, externalCfpData2.copy(close = Some(nowLDT.plusDays(10))))(ctx).unsafeRunSync()
