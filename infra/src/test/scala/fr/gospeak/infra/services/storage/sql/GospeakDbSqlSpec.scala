...
class GospeakDbSqlSpec extends FunSpec with Matchers with BeforeAndAfterEach with RandomDataGenerator {
  ...
  describe("GospeakDbSql") {
    describe("User") {
      it("should create and retrieve a user") {
        ...
        db.createUser(userSlug, firstName, lastName, email).unsafeRunSync()
        db.getUser(email).unsafeRunSync().map(_.email) shouldBe Some(email)
      }
      it("should fail on duplicate slug") {
        db.createUser(userSlug, firstName, lastName, email).unsafeRunSync()
        an[Exception] should be thrownBy db.createUser(userSlug, firstName, lastName, email2).unsafeRunSync()
      }
      it("should fail on duplicate email") {
        db.createUser(userSlug, firstName, lastName, email).unsafeRunSync()
        an[Exception] should be thrownBy db.createUser(userSlug2, firstName, lastName, email).unsafeRunSync()
      }
    }
    describe("Group") {
      it("should create and retrieve a group") {
        val user = db.createUser(userSlug, firstName, lastName, email).unsafeRunSync()
      ...
      it("should not retrieve not owned groups") {
        val user = db.createUser(userSlug, firstName, lastName, email).unsafeRunSync()
        val user2 = db.createUser(userSlug2, "A", "A", email2).unsafeRunSync()
  ...
  def createUserAndGroup(): IO[(User, Group)] = for {
    user <- db.createUser(userSlug, firstName, lastName, email)
