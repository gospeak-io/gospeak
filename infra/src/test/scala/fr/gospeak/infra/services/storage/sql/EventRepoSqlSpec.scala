  ...
  describe("EventRepoSql") {
    ...
    describe("Queries") {
      ...
      it("should build updateNotes") {
        val q = EventRepoSql.updateNotes(group.id, event.slug)(Some("notes"), user.id, now)
        check(q, s"UPDATE $table SET orga_notes=?, updated=?, updated_by=? WHERE e.group_id=? AND e.slug=?")
      }
...
object EventRepoSqlSpec {
  ...
  val table = "events e"
  val fields: String = mapFields("id, group_id, cfp_id, slug, name, start, max_attendee, allow_rsvp, description, orga_notes, venue, talks, tags, published, meetupGroup, meetupEvent, created, created_by, updated, updated_by", "e." + _)
