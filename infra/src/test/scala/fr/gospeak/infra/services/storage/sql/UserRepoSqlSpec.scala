...
class UserRepoSqlSpec extends RepoSpec {
  ...
  private val fields = "id, slug, first_name, last_name, email, email_validated, avatar, avatar_source, published, description, shirt, company, location, twitter, linkedin, phone, webSite, created, updated"
...
//        val q = update(user)
//        q.sql shouldBe "UPDATE users SET slug=?, first_name=?, last_name=?, email=?, published=?, updated=? WHERE id=?"
//        check(q)
//      }
