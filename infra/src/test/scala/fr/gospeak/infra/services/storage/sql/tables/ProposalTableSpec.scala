  ...
  describe("ProposalTable") {
    describe("insert") {
      it("should generate the query") {
        ...
        q.sql shouldBe "INSERT INTO proposals (id, talk_id, cfp_id, event_id, title, duration, status, description, speakers, created, created_by, updated, updated_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
        check(q)
      }
    }
    ...
    describe("selectOne") {
      it("should generate query for proposal id") {
        ...
        q.sql shouldBe "SELECT id, talk_id, cfp_id, event_id, title, duration, status, description, speakers, created, created_by, updated, updated_by FROM proposals WHERE id=?"
        check(q)
      }
      it("should generate query for talk and cfp ids") {
        val q = selectOne(talk.id, cfp.id)
        q.sql shouldBe "SELECT id, talk_id, cfp_id, event_id, title, duration, status, description, speakers, created, created_by, updated, updated_by FROM proposals WHERE talk_id=? AND cfp_id=?"
        check(q)
      }
    }
    describe("selectPage") {
      it("should generate query for a talk") {
        ...
        s.sql shouldBe
          "SELECT c.id, c.group_id, c.slug, c.name, c.description, c.created, c.created_by, c.updated, c.updated_by, " +
            "p.id, p.talk_id, p.cfp_id, p.event_id, p.title, p.duration, p.status, p.description, p.speakers, p.created, p.created_by, p.updated, p.updated_by " +
      ...
      it("should generate query for a cfp") {
        ...
        s.sql shouldBe "SELECT id, talk_id, cfp_id, event_id, title, duration, status, description, speakers, created, created_by, updated, updated_by FROM proposals WHERE cfp_id=? ORDER BY created DESC OFFSET 0 LIMIT 20"
      ...
      it("should generate query for a cfp and status") {
        ...
        s.sql shouldBe "SELECT id, talk_id, cfp_id, event_id, title, duration, status, description, speakers, created, created_by, updated, updated_by FROM proposals WHERE cfp_id=? AND status=? ORDER BY created DESC OFFSET 0 LIMIT 20"
    ...
    describe("selectAll") {
      it("should generate the query") {
        ...
        q.sql shouldBe "SELECT id, talk_id, cfp_id, event_id, title, duration, status, description, speakers, created, created_by, updated, updated_by FROM proposals WHERE id IN (?) "
        check(q)
      }
    }
  }
